...
let streetViewMan = null;
let isStreetViewActive = false;
let isMapTilted = false;
let transportLayer = null;
let isTransportVisible = false;
...
function showUserProfile() {
    ...
    profileContainer.innerHTML = `
        <div class="profile-avatar" onclick="toggleProfileMenu()">${currentUser.name.substring(0, 1)}</div>
        <div class="profile-menu" id="profileMenu">
            <div class="menu-item" onclick="showProfile()"><i class="fas fa-user"></i> Профиль</div>
            <div class="menu-item" onclick="showSettings()"><i class="fas fa-cog"></i> Настройки</div>
            <div class="menu-item" onclick="showFriends()"><i class="fas fa-users"></i> Друзья</div>
            <div class="menu-item" onclick="showSavedPlaces()"><i class="fas fa-bookmark"></i> Избранное</div>
            <div class="menu-item" onclick="showHistory()"><i class="fas fa-history"></i> История</div>
            <div class="menu-item" onclick="logout()"><i class="fas fa-sign-out-alt"></i> Выйти</div>
        </div>
    `;
    document.querySelector('.header-right').appendChild(profileContainer);
}
...
function setupLoginButton() {
    ...
    loginBtn.innerHTML = `
        <div class="login-container">
            <button class="login-btn" onclick="goToLoginPage()">
...
function showProfile() {
    ...
    document.querySelector('.avatar-placeholder').innerHTML = currentUser.avatar;
    document.getElementById('profileRegistrationDate').textContent = currentUser.registrationDate;
...
function saveProfile() {
    ...
    if (userIndex !== -1) {
        ...
        users[userIndex].avatar = newName.substring(0, 1).toUpperCase();
        localStorage.setItem('users', JSON.stringify(users));
...
function setupRouteButton() {
    const routeButton = new ymaps.control.Button({
        data: {
            content: '<i class="fas fa-route"></i>',
    ...
    routeButton.events.add('click', function() {
        if (currentPlace) {
            ...
            alert('Выберите место назначения, кликнув по маркеру на карте');
        }
    });

    map.controls.add(routeButton, {float: 'right', floatIndex: 200});
}

function setupStreetViewButton() {
    const streetViewButton = new ymaps.control.Button({
        data: {
            content: '<i class="fas fa-street-view"></i>',
            title: 'Режим просмотра улиц'
        },
        options: {
            selectOnClick: true,
            maxWidth: 40
    ...
    streetViewButton.events.add('click', function() {
        toggleStreetView();
    });

    map.controls.add(streetViewButton, {float: 'right', floatIndex: 190});
}
...
function toggleStreetView() {
    ...
    if (isStreetViewActive) {
        if (!streetViewMan) {
            streetViewMan = new ymaps.Placemark(map.getCenter(), {}, {
                preset: 'islands#blueManIcon',
                draggable: true
            });
            map.geoObjects.add(streetViewMan);
        } else {
            streetViewMan.options.set('visible', true);
        }

        streetViewMan.events.add('dragend', function() {
            const coords = streetViewMan.geometry.getCoordinates();
            showStreetView(coords);
        });

        showStreetView(streetViewMan.geometry.getCoordinates());
        showNotification('Режим просмотра улиц включен. Перетащите человечка на карту.');
    } else {
        if (streetViewMan) {
            streetViewMan.options.set('visible', false);
        }
        hideStreetView();
        showNotification('Режим просмотра улиц выключен');
    }
}
...
function showStreetView(coords) {
    ...
    streetViewContainer.innerHTML = `
        <div class="street-view-header">
            <h3>Просмотр улиц</h3>
            <button onclick="hideStreetView()"><i class="fas fa-times"></i></button>
        </div>
        <div class="street-view-content">
            <img src="https://maps.googleapis.com/maps/api/streetview?size=600x400&location=${coords[0]},${coords[1]}&fov=90&heading=235&pitch=10&key=AIzaSyD9RxIAyohj1Z7RgDZUyPZx0VZQm0XwF4M" alt="Street View">
        </div>
    `;
}
...
function setupTiltButton() {
    const tiltButton = new ymaps.control.Button({
        data: {
            content: '<i class="fas fa-angle-double-up"></i>',
            title: 'Наклонить карту'
        },
        options: {
            selectOnClick: true,
            maxWidth: 40
    ...
    tiltButton.events.add('click', function() {
        toggleMapTilt();
    });

    map.controls.add(tiltButton, {float: 'right', floatIndex: 180});
}
...
function toggleMapTilt() {
    ...
    if (isMapTilted) {
        map.options.set('tilt', 45);
        showNotification('3D вид включен');
    } else {
        map.options.set('tilt', 0);
        showNotification('2D вид включен');
    }
}

function setupTransportButton() {
    const transportButton = new ymaps.control.Button({
        data: {
            content: '<i class="fas fa-bus"></i>',
            title: 'Транспорт'
        },
        options: {
            selectOnClick: true,
    ...
    transportButton.events.add('click', function() {
        toggleTransport();
    });

    map.controls.add(transportButton, {float: 'right', floatIndex: 170});
}
...
function toggleTransport() {
    ...
    if (isTransportVisible) {
        showTransport();
        showNotification('Транспорт отображается на карте');
    } else {
        hideTransport();
        showNotification('Транспорт скрыт');
    }
}
...
function showTransport() {
    ...
    const transportTypes = [
        {type: 'bus', color: '#FF0000'},
        {type: 'tram', color: '#0000FF'},
        {type: 'trolleybus', color: '#00FF00'}
    ];
    
    transportTypes.forEach(transport => {
        for (let i = 0; i < 5; i++) {
            const coords = [
                map.getCenter()[0] + (Math.random() - 0.5) * 0.02,
                map.getCenter()[1] + (Math.random() - 0.5) * 0.02
            ];
            
            const transportIcon = new ymaps.Placemark(coords, {
                hintContent: `${transport.type} №${Math.floor(Math.random() * 100) + 1}`
            }, {
                preset: `islands#${transport.type}Icon`,
                iconColor: transport.color
            });
            
            map.geoObjects.add(transportIcon);
            transportObjects.push(transportIcon);
            
            moveTransport(transportIcon);
        }
    });
}

function moveTransport(transport) {
    const interval = setInterval(() => {
        ...
        const currentCoords = transport.geometry.getCoordinates();
        const newCoords = [
            currentCoords[0] + (Math.random() - 0.5) * 0.0005,
            currentCoords[1] + (Math.random() - 0.5) * 0.0005
        ];
        transport.geometry.setCoordinates(newCoords);
    }, 1000);
}

function hideTransport() {
    transportObjects.forEach(transport => {
        map.geoObjects.remove(transport);
    });
    transportObjects = [];
}
...
function setupTrafficButton() {
    const trafficButton = new ymaps.control.Button({
        data: {
            content: '<i class="fas fa-car-crash"></i>',
            title: 'Пробки'
        },
        options: {
            selectOnClick: true,
    ...
    map.controls.add(trafficButton, {float: 'right', floatIndex: 160});
}
...
function showMarkerContextMenu(coords) {
    ...
    contextMenu.innerHTML = `
        <div class="context-menu-item" onclick="createMarkerAt(${coords[0]}, ${coords[1]})">
            <i class="fas fa-map-marker-alt"></i> Создать маркер
        </div>
        <div class="context-menu-item" onclick="buildRouteFromHere(${coords[0]}, ${coords[1]})">
            <i class="fas fa-route"></i> Построить маршрут отсюда
        </div>
        <div class="context-menu-item" onclick="addToSavedPlaces(${coords[0]}, ${coords[1]})">
            <i class="fas fa-bookmark"></i> Сохранить место
        </div>
...
function buildRouteFromHere(lat, lng) {
    if (!userLocation) {
        alert('Для построения маршрута необходимо определить ваше местоположение');
        return;
    }
    
    const fromCoords = [lat, lng];
    const toCoords = userLocation;
...
function addToSavedPlaces(lat, lng) {
    if (!currentUser) {
        alert('Для сохранения мест необходимо войти в систему');
        return;
    }
    ...
    const newPlace = {
        ...
        properties: {
            name: `Сохраненное место ${savedPlaces.length + 1}`,
            description: 'Пользовательское сохраненное место',
            category: 'custom',
            subcategory: 'saved',
            popularity: 50,
            isHeritage: false,
            address: 'Пользовательское место',
            phone: '',
            hours: '',
            rating: '5.0',
            reviewsCount: 0,
    ...
    showNotification('Место сохранено');
    hideContextMenu();
}
...
function ratePlace(placeId, rating) {
    if (!currentUser) {
        alert('Для оценки места необходимо войти в систему');
        return;
    }
...
function showSavedPlaces() {
    if (!currentUser) {
        alert('Для просмотра сохраненных мест необходимо войти в систему');
        return;
    }
...
function showFriends() {
    if (!currentUser) {
        alert('Для просмотра друзей необходимо войти в систему');
        return;
    }
...
function addFriend() {
    if (!currentUser) {
        alert('Для добавления друзей необходимо войти в систему');
        return;
    }
...
function createCustomMarker() {
    if (!currentUser) {
        alert('Для создания маркеров необходимо войти в систему');
        return;
    }
...
function showCollaboration() {
    if (!currentUser) {
        alert('Для совместной работы необходимо войти в систему');
        return;
    }
